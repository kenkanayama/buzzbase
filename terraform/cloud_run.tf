...
# -----------------------------------------------------------------------------
# Cloud Run Service (審査用環境)
# -----------------------------------------------------------------------------

resource "google_cloud_run_v2_service" "buzzbase_review" {
  name     = "buzzbase-review"
  location = var.region
  ingress  = "INGRESS_TRAFFIC_ALL"
...
# 審査用環境: 未認証ユーザーからのアクセスを許可
resource "google_cloud_run_v2_service_iam_member" "review_public_access" {
  project  = var.project_id
  location = var.region
  name     = google_cloud_run_v2_service.buzzbase_review.name
  role     = "roles/run.invoker"
  member   = "allUsers"
}
