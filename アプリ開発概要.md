...
* 投稿日（JST基準）から **1〜7日目** に該当する投稿を対象に、Instagram APIからインサイトデータを取得。
* 取得タイミング:
  * **0日目（投稿登録時）**: `fetchPostInsightsImmediate` で即座に取得
  * **1〜7日目**: バッチ処理で毎日取得（最大8回のインサイトデータ取得）
  * **7日目のバッチ処理後**: ステータスを `measured` に変更（計測完了）
* 取得データ（リーチ、保存数、再生数、いいね数、コメント数など）を Firestore の `prPosts` コレクションに書き込む（最新データで上書き）。
