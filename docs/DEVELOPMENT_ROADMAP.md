...
### Meta社のInstagram API審査通過（2026年1月1日）

Instagram Graph APIの利用が正式に承認され、本番環境でのAPI利用が可能になりました。
これにより、以下の機能が本番環境で利用可能になりました：

- ✅ Instagramアカウント連携機能
- ✅ Instagram投稿一覧取得機能
- ✅ PR投稿登録機能
- ✅ バッチ処理による数値データ取得機能（Phase 7）
- ✅ アクセストークン自動更新機能（Phase 6）

**次のステップ**: Phase 5（投稿一覧フィルタリング機能）の実装を優先的に進めることを推奨します。
...
- [x] アクセストークン更新関数の作成
- [x] Instagram Graph API のトークンリフレッシュ処理
- [x] 更新されたトークンを Firestore に保存する処理
- [x] エラーハンドリング（更新失敗時の通知等）

#### 6-2. Cloud Scheduler の設定

- [x] トークン更新バッチのスケジュール設定（毎日2:00 JST）
- [x] 実行頻度の決定（トークン有効期限に基づく）
...
- [x] エラーログ出力（更新失敗時の詳細ログ）
- [ ] 更新失敗時のユーザー通知（メール or アプリ内通知）※将来の拡張
- [ ] 再連携を促すUI表示※将来の拡張
...
> 
> - **実行時刻**: 毎日 2:00 JST
> - **更新判定**: 有効期限まで45日以内のトークンを更新対象とする
>   - 例：12月12日に期限切れのトークン → 10月28日以降に更新対象となる
> 
> **処理フロー**:
> 1. Cloud Scheduler → Pub/Sub → Cloud Functions
> 2. `instagramAccounts`コレクションから全アカウントを取得
> 3. 各アカウントの`tokenExpiresAt`をチェック
> 4. 45日以内に期限切れになるトークンをリフレッシュ
> 5. リフレッシュ後のトークンが短期トークンの場合は長期トークンに交換
> 6. Firestoreに保存
> 
> **エラーハンドリング**:
> - 更新失敗時はエラーログを出力
...
投稿から7日目までの数値データを毎日自動取得するバッチ処理を実装する。
...
- [x] 実行タイミングの決定：投稿日から1〜7日目まで毎日23:00 JSTにデータ取得
...
>   - 例：12月5日に投稿された投稿 → 12月6日〜12月12日の23:00バッチで毎日処理
>   - 12月12日（7日目）の処理後にステータスが 'measured' に変更される
> 
> **処理フロー**:
> 1. Cloud Scheduler → Pub/Sub → Cloud Functions
...
| No. | 項目 | 詳細 | ステータス |
|-----|------|------|-----------|
| 1 | バッチ処理タイミング | 投稿から1〜7日目まで毎日の取得時刻 | ✅ 完了（毎日23:00 JST） |
...
| 2025-12-24 | Phase 6（Instagramアクセストークン自動更新機能）実装完了 |
| 2026-01-01 | Meta社のInstagram API審査通過 - 本番環境でのAPI利用が可能に |
